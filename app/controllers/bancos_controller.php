...
class BancosController extends MasterDetailAppController {
	var $name='Bancos';
...
/*
	var $paginate = array(	'update' => '#content',
							...
							'limit' => 15,
							'order' => array('Banco.arcveart' => 'asc'),
						);
*/
	...
	function edit($id = null) {
		...
		if (!empty($this->data)) {
			if ($this->Banco->save($this->data)) {
				...
				$dates = $this->Banco->find(array('id'=>$id),array('created','modified'));
				$this->data['Banco']['created'] = $dates['Banco']['created'];
				$this->data['Banco']['modified'] = $dates['Banco']['modified'];
		...
		$divisas = $this->Banco->Divisa->find('list', array('fields' => array('Divisa.id', 'Divisa.dicve')));
		$this->set(compact('divisas'));
		$temporadas = $this->Banco->Temporada->find('list', array('fields' => array('Cliente.id', 'Cliente.clnom')));
...
//		}
	//	$this->Banco->recursive = 0;
		...
		$this->set('bancos', $this->paginate($filter));
//		parent::index();
	}
