...
class BitacorasController extends MasterDetailAppController {
	var $name='Bitacoras';
...
/*
	var $paginate = array(	'update' => '#content',
							...
							'limit' => 15,
							'order' => array('Bitacora.arcveart' => 'asc'),
						);
*/
	...
	function edit($id = null) {
		...
		if (!empty($this->data)) {
			if ($this->Bitacora->save($this->data)) {
				...
				$dates = $this->Bitacora->find(array('id'=>$id),array('created','modified'));
				$this->data['Bitacora']['created'] = $dates['Bitacora']['created'];
				$this->data['Bitacora']['modified'] = $dates['Bitacora']['modified'];
				$this->Session->setFlash(__('transaction_could_not_be_saved', true));
			}
		}
		if (empty($this->data)) {
			$this->data = $this->Bitacora->read(null, $id);
		}
		$divisas = $this->Bitacora->Divisa->find('list', array('fields' => array('Divisa.id', 'Divisa.dicve')));
		$this->set(compact('divisas'));
		$temporadas = $this->Bitacora->Temporada->find('list', array('fields' => array('Cliente.id', 'Cliente.clnom')));
	...
	**********************************************************************************************************/
	function createReport()
	{
		...
		else
		{
			...
			*/
			$models =	Array ("Bitacora",'Marca','Linea','Temporada');
	...
	function Monitor() {
		$this->Bitacora->recursive = 1;
		$this->data = $this->Bitacora->readAll();			
	}
}
