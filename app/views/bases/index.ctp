...
<?php 
echo $form->create('Base', array('inputDefaults' => array(
															'label' => false,
															'div'   => false
															))
															);
?>
	<table id="datagrid" class="table table-bordered table-striped table-condensed table-hover">
		...
		foreach ($items as $item):
			$class = null;
			$thisID=trim($item['Base']['id']);
		?>
			<tr id="<?php echo $thisID?>" class="t-row">
				<td class="cveart"><?php echo $item['Base']['cve']; ?></td>
				<td class=""><?php echo $item['Base']['descrip'];?></td>
				<td class="datetime"><?php echo $item['Base']['modified'];?></td>
				<td class="st"><?php echo $item['Base']['st']; ?></td>
				<td class="id"><?php echo $item['Base']['id']; ?></td>
			</tr>
		<?php endforeach; ?>
		</tbody>
	</table>
...
<?php echo $this->Element('MasterDetailIndexPaging',array('MyController'=>$this->name,'MyModel'=>'Base','MyRowClickAction' => 'edit')); ?>
