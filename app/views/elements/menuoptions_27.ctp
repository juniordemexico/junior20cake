<ul>
	<li><a class="returnFalse" href="">Habilitación</a>	
		<ul>
			<li class="divider dividerhead"><a class="returnFalse" href="">Catálogos</a>
			<li><a id="articulosLinkCheck" href="/Materiales" target="_new">Materiales</a></li>
			<li><a id="articulospreciosLinkCheck" href="/Familias" target="_new">Familias</a></li>
			<li><a id="proveedorescostosLinkCheck" href="/Proveedores/costos" target="_new">Costos por Proveedor</a></li>
			<li><a id="articulospreciosLinkCheck" href="/Explosiones" target="_new">Explosiones</a></li>
			<li class="divider dividerhead"><a id="operacionesLink" href="">Operaciones</a></li>
			<li><a id="bodegamaterialesLinkCheck" href="/Bodegamateriales" target="_new">Bodega Materiales</a></li>
			<li><a id="pedidosLinkCheck" href="/Materialmovimientos" target="_new">Entradas / Salidas</a></li>
			<li><a id="ventaexposLinkCheck" href="/Ventaexpos" target="_new"></a></li>
			<li class="divider dividerhead"><a id="consultasLink" href="">Consultas</a></li>
			<li><a id="facturaselectronicasLinkCheck" href="/Materiales/Existencias" target="_new">Existencias</a></li>
			<li><a id="facturaselectronicasLinkCheck" href="/Materiales/Kardex" target="_new">Kardex</a></li>
		</ul>
	</li>

	<li><a class="returnFalse" href="">Ver</a>
		<ul>		
			<li><a id="cascadeLink" href="">Ordenar en Cascada</a></li>
			<li><a id="tileLink" href="">Ordenar en Cuadrícula</a></li>
			<li class="divider"><a id="minimizeLink" href="">Minimizar Todo</a></li>
			<li><a id="closeLink" href="">Cerrar Todo</a></li>
		</ul>
	</li>
	<li><a class="returnFalse" href="">Herramientas</a>
		<ul>
		<li class="divider dividerhead"><a class="returnFalse" href="">Operación</a></li>						
		<li class="divider dividerhead"><a class="returnFalse" href="">Widgets</a></li>						
			<li><a id="clockLinkCheck" href="plugins/coolclock/">Widget: Clock</a></li>
			<li><a id="calculatorLinkCheck" href="plugins/calculator/">Widget: Calculator</a></li>
		<li class="divider dividerhead"><a class="returnFalse" href="">Escritorio</a></li>						
		<li><a id="saveWorkspaceLink" href="">Guardar Escritorio</a></li>
		<li><a id="loadWorkspaceLink" href="">Cargar Escritorio</a></li>
		<li class="divider"><a id="toggleEffectsLinkCheck" href="#">Usar Efectos</a></li>				
		</ul>
	</li>
</ul>
