...
<?php 
echo $form->create('Invfisicodetail', array('inputDefaults' => array(
															'label' => false,
															'div'   => false
															))
															);
	...
	<table id="datagrid" class="table table-bordered table-striped table-condensed table-hover">
		...
		?>
			<tr id="<?php echo $thisID?>" cve="<?php echo $item['Articulo']['arcveart']; ?>" class="t-row">
				...
				<td class="precio" title="<?php echo $this->Number->precision($item[0]['conteo_1'],0); ?> pzas en <?php echo $this->Number->precision($item[0]['marbetes_conteo_2'],0); ?> marbetes."><?php echo $this->Number->precision($item[0]['conteo_2'],0); ?></td>
				<td class="precio">00.00<?php //echo $this->Number->precision($item[0]['existencia'],0); ?></td>
