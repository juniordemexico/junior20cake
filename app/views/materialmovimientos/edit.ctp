...
//	localStorageService.clearAll();
	...
	$scope.$watch($scope.base.controller+$scope.base.action+'_details', function(value) {
		localStorageService.add($scope.base.controller+$scope.base.action+'_details', angular.toJson(value));
		$scope.details = angular.fromJson(localStorageService.get($scope.base.controller+$scope.base.action+'_details'));
	});

	if( !$scope.details.length>0 ) { //*typeof localStorageService.get('details') != 'undefined'*/ ) {
		$scope.details=angular.fromJson(localStorageService.get($scope.base.controller+$scope.base.action+'_details'));
	}
	else {
//		localStorageService.add($scope.base.controller+$scope.base.action+'_details', angular.toJson($scope.details));
	}
	...
	$scope.save = function() {
		...
		$scope.theDataToPost=serializedData;
