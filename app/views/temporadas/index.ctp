...
<?php 
echo $form->create('Temporada', array('inputDefaults' => array(
															'label' => false,
															'div'   => false
															))
															);
?>
	<table id="datagrid" class="table table-bordered table-striped table-condensed table-hover">
		<thead>
			...
			<tr class="row-labels">
				<th class=""><?php echo $this->Paginator->sort('Clave','tecve'); ?></th>
		...
		foreach ($temporadas as $item):
			$class = null;
			$thisID=trim($item['Temporada']['id']);
		?>
			<tr id="<?php echo $thisID?>" class="t-row">
				<td class=""><?php echo $item['Temporada']['tecve']; ?></td>
				<td class="id"><?php echo $item['Temporada']['id']; ?></td>
			</tr>
		<?php endforeach; ?>
		</tbody>
	</table>
...
<?php echo $this->Element('MasterDetailIndexPaging',array('MyController'=>$this->name,'MyModel'=>'Temporada','MyRowClickAction' => 'edit')); ?>
