 ...
 * The strings are found between <cake<cake:nocache><cake<cake:nocache> in views
 *
 * @var array
 * @access private
 */
 ...
 * The strings are any content inside <cake<cake:nocache><cake<cake:nocache> and includes the tags in views
 *
 * @var array
 * @access private
 */
 ...
 */
	function cache($file, $out, $cache = false) {
		...
		if ($cacheTime != '' && $cacheTime > 0) {
			$out = preg_replace_callback('/<cake\<cake:nocache>/', array($this, '_replaceSection'), $out);
 ...
 */
	function __parseFile($file, $cache) {
		...
		preg_match_all('/(<cake:nocache:\d{3}>(?<=<cake:nocache:\d{3}>)[\\s\\S]*?(?=<\/cake<cake:nocache>)<\/cake<cake:nocache>)/i', $cache, $outputResult, PREG_PATTERN_ORDER);
		preg_match_all('/(?<=<cake<cake:nocache>)([\\s\\S]*?)(?=<\/cake<cake:nocache>)/i', $file, $fileResult, PREG_PATTERN_ORDER);
 ...
 * @return string with all replacements made to <cake<cake:nocache><cake<cake:nocache>
 * @access private
 */
