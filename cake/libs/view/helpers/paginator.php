 ...
 */
	function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			$out .= $this->Html->tag($tag, $this->link($params['page'], array('page' => $params['page']), $options), array('class' => 'active'));
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i), $options), array('class' => 'active'));
				} else {
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i), $options));
				}
				if ($i != $params['pageCount']) {
